...
class MenuController():
    ...
    def find_data_main(self)->'ContactList':
        ...
        if value_dict!={"id":""} and data_from_id is None:
            ...
            try:
                ...
                input_all_fields = PhonebookView.input_message(INPUT_MESSAGE["all_fields"])
                if input_all_fields:
    ...
    def quit_phonebook_main(self) -> str:
        ...
        if self.contactlist != self.old_contactlist:
            input_data=PhonebookView.input_yn_message(INPUT_MESSAGE["yn_save"])
    ...
    def load_phonebook_data(self):
        try:
            ...
            PhonebookView.display_error(f"{self.filename} не соответствует требованиям: {e}")
        except NonUniqueId as e:
            PhonebookView.display_error(f"{self.filename} не соответствует требованиям: {e}")
        except InvalidHeaders as e:
            PhonebookView.display_error(f"{self.filename} не соответствует требованиям: {e}")
        except PhoneDataFileNotFoundError as e:
            PhonebookView.display_error(f'Телефонная книга {self.filename} не найдена. Создаем.\n')
            input_data=PhonebookView.input_yn_message(INPUT_MESSAGE["yn_random"])
            if input_data in ["N", "n"]:
                self.contactlist=ContactList()
            if input_data in ["Y", "y"]:
                try:
                    self.contactlist = ContactList().generate_random(10)
                    PhonebookView.display_message(OUTPUT_MESSAGE["random_input"])
