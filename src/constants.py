...
MENU_DICT = {
    ...
    "5": {"menu_item": "Удалить контакт", "def_name": "delete_data_main"},
...
word_pattern=r'[a-zа-яё]+(?:-[a-zа-яё]+)*' # буквы, потом опционально тире с буквами
pattern=r"{}".format(f'^{word_pattern}[ ]*({word_pattern})*$')
CHECK_PATTERNS_DICT={
    "surname": {"pattern": pattern, "hint":"Допустимы буквы, пробел и - внутри."}, 
    "name": {"pattern": pattern, "hint":"Допустимы буквы, пробел и - внутри."}, 
